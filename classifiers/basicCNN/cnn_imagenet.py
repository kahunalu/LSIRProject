...
class covnet:
	...
	def create_splits(self, label_folder, image_folder, ext, iteration, test, file_id):
		...
		if test:
			...
			np.save('cnn_imagenet_test_file'+str(file_id), self.test_data)
	...
	def SGD(self, epochs, mini_batch_size, eta, lmbda=0.0, test=False, file_id=0):
		...
		if test:
			...
			np.save("cnn_imagenet_predictions"+str(file_id), np.asarray(test_accuracy))
...
np.save("cnn_imagenet_params_pt1"+str(file_id), covnet.params[:(params_split_length)])
np.save("cnn_imagenet_params_pt2"+str(file_id), covnet.params[(params_split_length):(2*params_split_length)])
np.save("cnn_imagenet_params_pt3"+str(file_id), covnet.params[(2*params_split_length):(3*params_split_length)])
np.save("cnn_imagenet_params_pt4"+str(file_id), covnet.params[(3*params_split_length):(4*params_split_length)])
