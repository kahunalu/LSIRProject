...
class covnet:
	...
	def create_splits(self, label_folder, image_folder, ext, iteration, test):
		...
		if test:
			...
			np.save('dnn_imagenet_test_file', self.test_data)
	...
	def SGD(self, epochs, mini_batch_size, eta, lmbda=0.0, test=False):
		...
		if test:
			...
			np.save('results_dnn_imagenet', test_accuracy_array)
...
for i in range(0,6):	
	covnet.create_splits(
		...
		image_folder="/home/mclaren1/seng/LSIRProject/data/imagenet/data_folder/",
		ext=".dat",
		iteration=i,
		test=False
	)
...
np.save('dnn_imagenet_params_'+str(i), covnet.params)
